...
public class BaseApiService2 {
    ...
    //protected Retrofit sRetrofit;
    //private ApiServiceComponent apiServiceComponent;

    public BaseApiService2() {
        ApiServiceComponent apiServiceComponent = ApiEntryPoint.getInstance().apiServiceComponent();
        if (apiServiceComponent != null) {
            configuration = apiServiceComponent.apiConfiguration();
            credentialsStorage = apiServiceComponent.credentialStorage();
            //sRetrofit = component.retrofit();
        }
    }
